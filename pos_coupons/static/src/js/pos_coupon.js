...
odoo.define('pos_coupon.pos_coupon', function(require) {
    ...
    models.load_fields('account.journal', 'requiere_cupon');

    screens.PaymentScreenWidget.include({
        click_paymentmethods: function(id) {
            console.log("### CLICK PAYMENT METHODS ###")
            var self = this;
            var cashregister = null;
            var orden = this.pos.get_order();
            // var aplica_comision = orden.get_aplica_comsion_tarjeta();
            ...
            if (cashregister.journal.requiere_cupon) {
                self.gui.show_popup('redeem_coupon_pago_popup_widget', {
                    'confirm' : function(monto){
                        orden.add_paymentline_with_coupon(cashregister, monto)
                        self.reset_input();
                ...
                // this.show_popup_comision_info({
                //     'comision_debit': this.format_currency_no_symbol(cashregister.journal.comision_debit),
                //     'comision_credit': this.format_currency_no_symbol(cashregister.journal.comision_credit),
                //     'pay_due': this.format_currency_no_symbol(orden.get_due()),
                //     'data': {},
                //     'aplica_comision': aplica_comision,
                //     'confirm': function(infos) {
                //         //merge infos to new paymentline
                //         orden.add_paymentline_with_comision(cashregister, infos);
                //     },
                // });
            }
            else {
    ...
    var CreateCouponPopupWidget = PopupWidget.extend({
        ...
        renderElement: function() {
            ...
            .done(function(result){
                console.log("#### RESULT DEFAULT VALUES VAUCHER ####");
                console.log(result);
    ...
    var RedeemPagoPopupWidget = PopupWidget.extend({
        template: 'RedeemPagoPopupWidget',
        ...
        renderElement: function() {
            ...
            this.$('#wk-redeem-coupons').click(function() {
                self.click_confirm();
                // var secret_code = self.$("#coupon_8d_code").val();
        ...
        click_confirm: function(){
            ...
            rpc.query({
                ...
                if (result.status)
                {
                    order.coupon_id = result.coupon_id;
                    order.wk_product_id = result.product_id;
                    var total_venta = order.get_total_with_tax();
                    if (result.value > total_venta){
                        order.wk_voucher_value = order.get_total_with_tax();
                    }
                    else{
                        order.wk_voucher_value = result.value;
                    }

                    self.gui.close_popup();
                    if( self.options.confirm ){
                        self.options.confirm.call(self, order.wk_voucher_value);
                    }
                }
                else
                {
    ...
    gui.define_popup({
        name: 'redeem_coupon_pago_popup_widget',
        widget: RedeemPagoPopupWidget
    });
    ...
    models.Order = models.Order.extend({
        ...
        add_paymentline_with_coupon: function(cashregister, monto) {
            console.log("### infos ###")
            ...
            // newPaymentline.wk_product_id = this.wk_product_id;
            newPaymentline.set_amount(Math.max(monto, 0));
            // this.paymentlines = [];
            ...
            // if(infos.card_monto > this.get_due()){
            //     this.pos.gui.show_popup('error',{
            //         'title': _t('Advertencia'),
            //         'body':  _t('El monto es mayor al total'),
            //     });
            //    return false;
            // }
        },
    ...
    models.PosModel = models.PosModel.extend({
        _save_to_server: function(orders, options) {
            ...
            console.log(self);
            console.log(wk_order);
            
            // for(var l=0; l < wk_order.paymentlines.length; l++){
            //     var pline = wk_order.paymentlines
            //     console.log("## LINEA ##")
            //     // console.log(pline[l]);
            //     console.log(pline.models[l]);
            // }
                ...
                if (server_ids) {
                    ...
                    if (wk_order != null) {
                        ...
                        for(var l=0; l < wk_order.paymentlines.length; l++){
                            ...
                            // console.log(pline[l]);
                            console.log(pline.models[l]);
                            var cashregister = pline.models[l].cashregister;
                            if (cashregister.journal.requiere_cupon){
                                wk_voucher_value = pline.models[l].amount;
                            }
                        }
                        ...
                        if (coupon_id && wk_product_id){
                            rpc.query({
                                model: 'voucher.voucher',
                                method: 'pos_create_histoy',
                                args :  [coupon_id, wk_voucher_value, server_ids[0], wk_order.orderlines.models[0].id, client_id],
                            }).fail(function(unused, event) {
                                self.gui.show_popup('error', {
                                    'title': _t('Error !!!'),
                                    'body': _t("Connection Error. Try again later !!!"),
                        ...
                        if(!wk_voucher_value && wk_product_id == 0){
                            $('.receipt-screen.screen span.button.next').show();
                        }
