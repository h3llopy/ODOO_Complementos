odoo.define('pos_promotional_discounts.models', function (require) {
	...
	models.load_models([
		{
			...
			loaded: function(self,promo_messages){
				...
				self.db.all_promo_message_by_id = [];
				_.each(self.db.all_promo_message, function(data){
					self.db.all_promo_message_by_id[data.id] = data
				})
			},
        },{
			...
			loaded: function(self,result){
                ...
                _.each(self.db.buy_x_get_y_qty, function(data){
					self.db.buy_x_get_y_qty_by_id[data.product_x_id[0]] = data
                })
			},
		},{
			...
			model: 'combo.products',
			fields: [],
			loaded: function(self,result){
                self.db.combo_products = result;
			},
		},
    ]);
    ...
    models.Orderline = models.Orderline.extend({
        initialize: function(attr,options){
            ...
            self.is_buy_combo_get_free_product = options.is_buy_combo_get_free_product || false
            ...
            self.is_combo_product = options.is_combo_product || false
        ...
        wk_offer_tag: function(){
            ...
            else if (this.is_discounted_product || this.is_buy_x_get_discount_on_y){
                return 'Discounted Product'
            }
            else if(this.is_buy_x_get_y_product){
                return 'Extra Product of ' + this.pos.db.product_by_id[this.related_product_id].display_name
            }
            else if(this.is_buy_x_get_y__qty_product){
                return 'Extra Product of ' + this.pos.db.product_by_id[this.related_product_id].display_name
            }
            else if(this.is_combo_product){
                return 'Is Combo Product'
            }
            else{
                return false
            }
        ...
        export_as_JSON:function(){
            ...
            loaded.is_combo_product = self.is_combo_product || false
        ...
        get_discount_val: function(product){
            var self = this;
            if (product){
                var discount_val = 0
                var flag = false
                ...
                // discount_val = self.pos.db.discounted_product_by_id[product.id].discount
                return discount_val
            }
        },
            ...
            _.each(offers, function(offer){
                if (!apply_offer){
                    if(offer.offer_type == 'discount_on_products'){
                        ...
                        // if(self.pos.db.discounted_product.length){
                ...
                if (!apply_offer){
                    if(offer.offer_type == 'buy_combo_get_product_free'){
                        if(self.pos.db.combo_products.length){
                            _.each(self.pos.db.combo_products, function(item){
                                _.each(item.combo_product_ids, function(combo_product_id){
                                    if (combo_product_id == product.id)
                                        apply_offer = 'buy_combo_get_product_free'
                                })
                            })
                        }
                    }
        ...
        get_all_promotions: function(order){
			var self = this;
			var all_promotions = self.pos.db.all_promo_message;
            ...
            if (order){
                ...
                _.each(all_promotions,function(promo_message){
                    ...
                    else if(promo_message.criteria_type == 'from_date_to_date'){
                        var current_date = moment(new Date()).locale('en').format("YYYY-MM-DD");
                        if(promo_message.from_date <= current_date && promo_message.to_date >= current_date){
                            promo_messages_dict[promo_message.id] = promo_message;
                        }
                    }
    ...
    models.Order = models.Order.extend({
        ...
        check_and_apply_discount_offer: function(apply_offer,order,product,options, active_line){
            ...
            if (offer_product_id){
                ...
                if (offer_product){
                    var offer_can_be_applied = self.can_apply_buy_x_get_discount_on_y(apply_offer,order,offer_product,options, active_line)
        ...
        can_apply_buy_x_get_discount_on_y: function(apply_offer,order,product,options, active_line){
            var self = this;
            var orderlines = order.get_orderlines();
            var count_qty = 0;
            var has_extra = false;
            if(apply_offer){
                ...
                if(self.pos.db.buy_x_get_discount_on_y_by_id[product.id]){
                    if(count_qty >= self.pos.db.buy_x_get_discount_on_y_by_id[product.id].qty_x){
                        return true
                    }
                    else{
                        orderlines.forEach(function(orderline){
        ...
        can_apply_buy_x_get_y_qty: function(apply_offer,order,product,options, active_line){
            var self = this;
            var orderlines = order.get_orderlines();
            var count_qty = 0;
            var has_extra = false;
            if(apply_offer){
                ...
                if(self.pos.db.buy_x_get_y_qty_by_id[product.id]){
                    if(count_qty >= self.pos.db.buy_x_get_y_qty_by_id[product.id].qty_x){
                        return true
                    }
                    else{
                        orderlines.forEach(function(orderline){
        ...
        buy_x_get_discount_on_y: function(apply_offer,order,product,options,active_line){
            var self = this;
            var count_qty = 0
            var found = false
            ...
            var discount = 0
            if(self.pos.db.buy_x_get_discount_on_y_by_id[product.id]){
                buy_x_get_discount_on_y_qty = Math.floor(self.pos.db.buy_x_get_discount_on_y_by_id[product.id].qty_x)
                buy_x_get_discount_product = self.pos.db.product_by_id[self.pos.db.buy_x_get_discount_on_y_by_id[product.id].product_y_id[0]]
                discount = self.pos.db.buy_x_get_discount_on_y_by_id[product.id].discount
            }
        ...
        add_buy_x_get_y_qty: function(apply_offer,order,product,options,active_line){
            var self = this;
            var count_qty = 0
            var found = false
            var old_orderline = null;
            ...
            if(self.pos.db.buy_x_get_y_qty_by_id[product.id]){
                buy_x_get_y_product_qty = (Math.floor(count_qty/self.pos.db.buy_x_get_y_qty_by_id[product.id].qty_x) * self.pos.db.buy_x_get_y_qty_by_id[product.id].qty_y)
                buy_x_get_y_product = self.pos.db.product_by_id[self.pos.db.buy_x_get_y_qty_by_id[product.id].product_y_id[0]]
            }
        ...
        can_apply_buy_x_get_y: function(apply_offer,order,product,options, active_line){
            var self = this;
            var orderlines = order.get_orderlines();
            var count_qty = 0;
            var has_extra = false;
            if(apply_offer){
                ...
                if(self.pos.db.buy_x_get_y_by_id[product.id]){
                    if(count_qty >= self.pos.db.buy_x_get_y_by_id[product.id].qty_x){
                        return true
                    }
                    else{
                        orderlines.forEach(function(orderline){
        ...
        add_buy_x_get_y: function(apply_offer,order,product,options,active_line){
            var self = this;
            var count_qty = 0
            var found = false
            var old_orderline = null;
            ...
            var buy_x_get_y_product = null
            if(self.pos.db.buy_x_get_y_by_id[product.id]){
                buy_x_get_y_product_qty = Math.floor(count_qty/self.pos.db.buy_x_get_y_by_id[product.id].qty_x)
                buy_x_get_y_product = self.pos.db.product_by_id[self.pos.db.buy_x_get_y_by_id[product.id].product_y_id[0]]
            }
